<template>
  <section class="review-section">
    <header class="section-head">
      <div>
        <p class="eyebrow">企业审核</p>
        <h2>待审企业列表</h2>
        <p class="muted">核验企业信息后再通过，确保平台数据可信。</p>
      </div>
      <button class="outline" @click="loadPendingCompanies">刷新</button>
    </header>

    <div v-if="pendingCompanies.length" class="table-wrapper">
      <table class="table">
        <thead>
          <tr>
            <th>企业名称</th>
          ...
          <tr v-for="company in pendingCompanies" :key="company.id">
            ...
            <td>
              <span class="status-pill" :class="statusClass(company.auditStatus)">
                {{ formatStatus(company.auditStatus) }}
              </span>
            </td>
            <td class="actions">
              <button class="outline" @click="openDetail(company.id)">详情</button>
          ...
          <div class="detail-grid">
            ...
            <div>
              ...
              <span class="status-pill" :class="statusClass(detailDialog.company.auditStatus)">
                {{ formatStatus(detailDialog.company.auditStatus) }}
              </span>
            </div>
